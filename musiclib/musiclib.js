...
(function(){
  ...
  async function loadSongs(){
    try{
      ...
      const all=await Promise.all(jsons.map(f=>fetch(RAW_BASE+f.name+'?t='+Date.now(),{cache:'no-store'}).then(r=>r.json()).catch(()=>null)));
  ...
  function createMetronome(defaultBpm){
    ...
    toggle.onclick=()=>{
      if(_metroRunning){stopMetronome();}
      else{startMetronome(getBpm());}
    };
