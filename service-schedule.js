/**
 * service-schedule.js v16.0 — matrix + prayer meetings + instruments
 * fields:
 * month week type subtype leader worship prayerLeader praisePrayer memorialPrayer piano drums guitar bass reading note time
 */
(function () {
  'use strict';
  ...
  var COLS = [
    ...
    { key: 'note',           label: '备注 / 证道', kind: 'note'    }
  ];
  ...
  if (!document.getElementById('_cecp_matrix_v16')) {
    var st = document.createElement('style');
    st.id = '_cecp_matrix_v16';
    st.textContent = `
...
.cec-btn-tool:disabled{opacity:.45;cursor:not-allowed}
.cec-z2,.cec-z3{display:flex;align-items:center;gap:6px;padding:8px 12px;background:var(--soft2);border-bottom:1px solid var(--ln);flex-wrap:wrap}
.cec-zlbl{color:var(--tx2);font-size:12px;font-weight:800;margin-right:2px}
.cec-btn-week{height:30px;padding:0 12px;border-radius:9px;border:1px solid transparent;color:var(--tx2);background:transparent;font-size:12px;font-weight:700}
.cec-btn-week:hover:not(.on){background:var(--bg3);color:var(--tx)}
.cec-btn-week.on{background:var(--bg4);color:var(--tx);border-color:var(--ln2)}
.cec-btn-week.all.on{background:var(--tx);color:var(--bg);border-color:var(--tx)}
.cec-btn-type{height:28px;padding:0 10px;gap:6px;border-radius:999px;border:1px solid transparent;color:var(--tx2);background:transparent;font-size:12px;font-weight:700}
.cec-btn-type:hover:not(.on){background:var(--bg3);color:var(--tx)}
.cec-btn-type.on{color:var(--bg)}
.cec-btn-type.all.on{background:var(--tx);color:var(--bg);border-color:var(--tx)}
...
.cec-corner{background:rgba(255,255,255,.02);padding:14px 10px;text-align:center;color:var(--tx3);font-size:11px;font-weight:700;letter-spacing:.05em;position:sticky;top:0;z-index:3}
...
.cec-info{width:128px;min-width:128px;padding:14px 10px;text-align:left;background:rgba(255,255,255,.01);border-left:3px solid transparent}
.cec-wk{display:block;font-size:18px;line-height:1.04;font-weight:900;color:var(--tx);letter-spacing:-.03em}
.cec-mo{display:block;margin-top:5px;font-size:11px;font-weight:700;color:var(--tx3)}
.cec-sub{display:block;margin-top:6px;font-size:11px;font-weight:700;color:var(--tx2);line-height:1.2}
.cec-time{display:block;margin-top:3px;font-size:10px;font-weight:700;color:var(--tx3)}
.cec-tag{display:inline-flex;align-items:center;margin-top:10px;padding:4px 8px;border-radius:8px;font-size:11px;font-weight:800;line-height:1.1}
.cec-cell{min-width:118px;height:78px;padding:14px 10px;text-align:center;background:transparent}
.cec-cell:hover{background:rgba(255,255,255,.02)}
.cec-badges{display:flex;flex-wrap:wrap;justify-content:center;align-items:center;gap:8px;min-height:48px}
.cec-badge{display:inline-flex;align-items:center;justify-content:center;min-height:32px;padding:6px 12px;border-radius:7px;font-size:11px;font-weight:800;line-height:1.2;margin:0;white-space:nowrap;cursor:pointer;letter-spacing:.01em;transition:opacity .12s ease,transform .12s ease,box-shadow .12s ease,filter .12s ease;user-select:none;position:relative}
.cec-badge.lit{transform:translateY(-1px);filter:brightness(1.08);box-shadow:0 0 0 1px rgba(255,255,255,.16)}
.cec-badge.dim{opacity:.18}.cec-badge.locked{transform:translateY(-1px);filter:brightness(1.12);box-shadow:0 0 0 1.5px rgba(255,255,255,.26)}.cec-badge.ldim{opacity:.10}
.cec-lt .cec-badge.lit{box-shadow:0 0 0 1px rgba(0,0,0,.14)}.cec-lt .cec-badge.locked{box-shadow:0 0 0 1.5px rgba(0,0,0,.22)}
.cec-reading,.cec-note{display:flex;min-height:48px;flex-direction:column;align-items:center;justify-content:center;gap:5px}
...
.cec-empty{color:var(--tx3);font-size:15px;line-height:1}.cec-sep td{height:0;padding:0;background:transparent;border-right:none !important;border-bottom:1px solid var(--ln2) !important}
.cec-state{display:flex;align-items:center;justify-content:center;gap:12px;padding:60px 24px;color:var(--tx2);font-size:14px}
.cec-spin{width:18px;height:18px;border:2px solid var(--ln3);border-top-color:var(--tx2);border-radius:50%;animation:cecspin .7s linear infinite}@keyframes cecspin{to{transform:rotate(360deg)}}
.cec-err{padding:24px;color:#cf5e5e;font-size:14px;line-height:1.8}
@media(max-width:760px){.cec-z1,.cec-z2,.cec-z3{padding-left:10px;padding-right:10px}.cec-info{width:112px;min-width:112px;padding:12px 8px}.cec-wk{font-size:16px}.cec-cell{min-width:108px;height:72px;padding:10px 8px}.cec-chdr,.cec-corner{padding:12px 8px;font-size:11px}.cec-badges{gap:6px;min-height:42px}.cec-badge{min-height:28px;padding:5px 10px;font-size:10px;border-radius:6px}}
@media(max-width:560px){.cec-btn-month,.cec-btn-tool,.cec-btn-arr,.cec-btn-icon{height:32px}.cec-btn-arr,.cec-btn-icon{width:32px}.cec-btn-month{padding:0 11px;font-size:12px}.cec-btn-tool{padding:0 11px;font-size:12px}.cec-btn-week{height:28px;padding:0 10px;font-size:11px}.cec-btn-type{height:26px;padding:0 9px;font-size:11px}}
`;
  ...
  function boot(rows) {
    ...
    var tord = {'主日下午':0,'主日晚上':1,'青年团契':2,'祷告会':3};
    ...
    function render(i) {
      ...
      var key = mkeys[i];
      var weeksInMonth = [];
      (months[key] || []).forEach(function(r){ if (r.week && weeksInMonth.indexOf(r.week) < 0) weeksInMonth.push(r.week); });
      ...
      var hdr = '<th class="cec-corner">聚会</th>' + COLS.map(function(c){ return '<th class="cec-chdr">' + c.label + '</th>'; }).join('');
      ...
      var svcs = (months[key] || []).filter(function(s){
        ...
        var dt = (tord[a.type] || 9) - (tord[b.type] || 9); if (dt) return dt;
        var so = {'周三祷告会':0,'周六祷告会':1};
        return (so[a.subtype] || 9) - (so[b.subtype] || 9);
      });
      ...
      svcs.forEach(function(s, idx){
        ...
        var sub = s.subtype ? '<span class="cec-sub">' + esc(s.subtype) + '</span>' : '';
        var time = s.time ? '<span class="cec-time">' + esc(s.time) + '</span>' : '';
        ...
        tbody += '<tr>' + info + COLS.map(function(col){ return renderTd(col, s[col.key] || ''); }).join('') + '</tr>';
      });

      if (!svcs.length) tbody = '<tr><td colspan="' + (COLS.length + 1) + '" style="padding:48px;text-align:center;color:var(--tx3);font-size:14px">暂无数据</td></tr>';
      ...
      EL.innerHTML =
        ...
        '<div class="cec-wrap"><table class="cec-tbl" id="cTbl">' +
          cg + '<thead><tr>' + hdr + '</tr></thead><tbody>' + tbody + '</tbody>' +
        '</table></div>';
      ...
      if (z2el) z2el.querySelectorAll('.cec-btn-week').forEach(function(b){ b.addEventListener('click', function(){ activeWeek = this.dataset.w; render(i); }); });
      var z3el = EL.querySelector('#cZ3');
      if (z3el) z3el.querySelectorAll('.cec-btn-type').forEach(function(b){ b.addEventListener('click', function(){ activeType = this.dataset.t; render(i); }); });
  ...
  function renderTd(col, val) {
    ...
    return '<td class="cec-cell"><div class="cec-badges">' +
      String(val).split(/[\/\n]/).map(function(n){ n = n.trim(); return n ? mkB(n) : ''; }).filter(Boolean).join('') +
    '</div></td>';
  }
  ...
  function exportPng(key) {
    ...
    function run() {
      window.html2canvas(tbl, { backgroundColor: isDark ? '#111214' : '#f7f8fa', scale: 2, useCORS: true, logging: false })
        ...
        .catch(function(){ btn.innerHTML = '导出失败'; btn.disabled = false; });
    }
  ...
  function demo() {
    return [
      ...
      {month:'3月',week:'第一周',type:'主日晚上',subtype:'',leader:'林文宝',worship:'翁撒该/杨雪克/叶春叶',prayerLeader:'',praisePrayer:'',memorialPrayer:'',piano:'金紫涵/黄天丽',drums:'',guitar:'',bass:'',reading:'',note:'证道：金美德',time:''},
      {month:'3月',week:'第一周',type:'青年团契',subtype:'',leader:'',worship:'陈灵雅',prayerLeader:'',praisePrayer:'',memorialPrayer:'',piano:'叶群荣',drums:'王雅博',guitar:'张尚贤',bass:'刘仁利',reading:'',note:'证道：活动游戏',time:''},
      {month:'3月',week:'第一周',type:'祷告会',subtype:'周三祷告会',leader:'王皞阳',worship:'',prayerLeader:'',praisePrayer:'',memorialPrayer:'',piano:'',drums:'',guitar:'',bass:'',reading:'',note:'周三祷告会 · 21:00-22:00',time:'21:00-22:00'},
      {month:'3月',week:'第一周',type:'祷告会',subtype:'周六祷告会',leader:'彭永剑',worship:'',prayerLeader:'',praisePrayer:'',memorialPrayer:'',piano:'',drums:'',guitar:'',bass:'',reading:'',note:'周六祷告会 · 7:30-8:30',time:'7:30-8:30'}
    ];
  }
})();
