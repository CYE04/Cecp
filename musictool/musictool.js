...
/* ════════════════════════════════════════
   音符渲染
════════════════════════════════════════ */
/* 官方音乐字体(Bravura 子集, SIL OFL)——只含 segno/coda(U+E047/E048)，供 segno 记号用。懒注入到 iframe 文档一次。 */
...
function makeBarline(tok){
  ...
  if(tok==='coda'||tok==='segno'){
    ...
    }else{ ensureMusicFont(); var g=document.createElement('span');g.style.cssText='font-family:CecpMusic;font-size:20px;line-height:1;display:block;';g.textContent=String.fromCharCode(0xE047);top.appendChild(g); }
